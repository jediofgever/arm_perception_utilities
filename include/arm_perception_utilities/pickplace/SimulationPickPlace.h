 ...
 */
class SimulationPickPlace {
   private:
    // Default home pose , where robot returns
    geometry_msgs::Pose placing_pose;

    std::vector<geometry_msgs::Pose> placing_poses_vector;
   ...
   public:
     ...
     */
    void processSimulationPickPlaceCallback(const visualization_msgs::MarkerArrayConstPtr &detected_obj_pose_arrows);
};
#endif
